...
export default function Hero({ onExplore, openBookingModal }: HeroProps) {
  ...
  return (
    <section className="relative min-h-screen lg:h-screen lg:min-h-[820px] bg-luxury-bg text-[#F5F0E8] overflow-hidden flex flex-col lg:flex-row items-stretch pt-24 border-b border-luxury-border" id="hero-section">
      ...
      <div className="w-full lg:w-[58%] xl:w-[60%] flex items-center z-10 relative bg-luxury-bg px-6 sm:px-10 md:px-16 lg:pl-16 lg:pr-12 py-12 lg:py-0">
        <div className="flex flex-col md:flex-row gap-10 lg:gap-12 xl:gap-14 items-stretch justify-center w-full">
          ...
          <div className="w-full md:w-[220px] shrink-0 flex flex-col justify-center space-y-4 md:space-y-5 border-r border-gold/10 md:pr-6 lg:pr-8">
            {stats.map((stat, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-center space-x-4 p-2.5 pl-3 pr-6 bg-luxury-sec/80 border border-gold/15 hover:border-gold/35 transition-all duration-300 backdrop-blur-md rounded-full"
                  id={`hero-stat-card-${stat.id}`}
                >
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-gold/30 bg-gold/5 text-gold">
                    <IconComp className="h-5 w-5" />
                  </div>
                  <div>
                    <h3 className="font-serif text-xl sm:text-2xl font-normal leading-none text-white">{stat.value}</h3>
                    <p className="font-sans text-[8px] tracking-[0.12em] text-gold uppercase mt-1 font-bold">{stat.label}</p>
                  </div>
                </motion.div>
              );
            })}
            ...
            <motion.div
              ...
              className="flex items-center space-x-3.5 p-3.5 pl-4 pr-6 bg-luxury-sec/80 border border-gold/15 backdrop-blur-md rounded-full"
          ...
          <div className="flex-grow flex flex-col justify-center space-y-7 text-left max-w-xl">
            ...
            >
              <button
                onClick={onExplore}
                className="group relative flex items-center justify-center bg-gold px-8 py-4 text-[10.5px] font-bold tracking-[0.25em] text-luxury-bg uppercase transition-all duration-300 hover:bg-gold-soft hover:shadow-[0_0_30px_rgba(196,147,63,0.35)] rounded-full cursor-pointer"
                id="hero-explore-btn"
              >
              ...
              <button
                ...
                className="group flex items-center justify-center border border-white/20 hover:border-gold px-8 py-4 text-[10.5px] font-bold tracking-[0.25em] text-white uppercase transition-all duration-300 rounded-full cursor-pointer"
                id="hero-watch-btn"
              >
